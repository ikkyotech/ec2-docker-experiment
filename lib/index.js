...
module.exports = explicit({
    ...
    $: function (name, credentials, spec, access, imports, s3Info) {
        ...
        async.waterfall([
            ...
            require("./tasks/step--9-prepareInAndOut").bind(null, s3, s3Info && s3Info.in),
            require("./tasks/step-10-startExperiment").bind(null, imports),
            require("./tasks/step-11-uploadOutToS3").bind(null, version, s3, s3Info && s3Info.out),
            require("./tasks/step-12-cancelSpotRequest").bind(null, ec2),
            require("./tasks/step-13-showStatsForExperiment").bind(null, ec2)
        ], function (error) {
            if (error) {
                return console.error("ERROR while executing ec2 experiment: " + error);
            }
